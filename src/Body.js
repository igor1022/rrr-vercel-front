...
useEffect(() => {
  ...
  const posts = async() => {
    if (selected.length === 0) {
      const lenght = await axios.get('https://rrr-back-5615f255bc1e.herokuapp.com/get_length_bd');
      for (let i = 0; i < lenght.data.count_record; i++) {
        proms.push(axios.get(`https://rrr-back-5615f255bc1e.herokuapp.com/get_bd/${i}/`)); 
      }  
      ...
      setId(arr_id);
        ...
        const records = await axios.post('https://rrr-back-5615f255bc1e.herokuapp.com/get_length_bd_part', {selected});
